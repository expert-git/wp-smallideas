...
function siteorigin_theme_update_filter( $current ) {
	...
	if( empty($request) ){
		...
		$request = wp_remote_get(
			...
			),  'http://updates.siteorigin.com/premium/' . $theme . '/' ),
			array(
				'timeout'     => 10,
			)
		);
...
function siteorigin_theme_update_settings() {
	...
	$settings->add_section('premium', __('Premium', 'vantage'));
	$settings->add_field('premium', 'order_number', 'text', __('Order Number', 'vantage'), array(
